...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const manualRelayToDestChainResponse = await axelar.recovery.manualRelayToDestChain(txHash, senderOptions);
  console.log("manualRelayToDestChainResponse", manualRelayToDestChainResponse);

  const executeResponse = await axelar.recovery.execute(txHash);
